...
def load_data(graph_folder, test_size=1.0, random_state=42):
...
@torch.no_grad()
def evaluate(model, data, device, threshold=0.5, mask_type='train'):
    model.eval()
    metrics = {'f1': 0.0, 'flat_f1': 0.0}
...
def main(args):
    ...
    data = load_data(args.graph_folder, test_size=0.4, random_state=args.seed)
    ...
    # Final evaluation
    final_test_metrics = evaluate(model, data, device, mask_type='test')
